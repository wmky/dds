 ...
 */
/*
 * var database_value = "[" + "['mysql', 'mysql库', 'The database td']" + "]";
 * database_value = eval(database_value); var database_store = new
 * Ext.data.ArrayStore({ fields: ['id', 'discribe', 'content'], data :
 * database_value });
 */
...
saveSqlSubmit = function(id) {
	 ...
	 * 'upLoad.itm?method=uploadSqlFile', isUpload : true, form : sqlform,
	 * success:function(result){ responseStr = result.responseText;
	 * alert(responseStr); lm.hide(); if(responseStr=='9'){
	 * Ext.Msg.alert('提示','文件过大，请精简文件'); } if(responseStr=='1'){
	 * Ext.Msg.alert('提示','上传sql成功'); upload_view.hide(); }else
	 * if(responseStr=='0'){ Ext.Msg.alert('提示','上传sql失败'); } }, failure:
	 * function(result) { lm.hide(); Ext.Msg.alert('提示','上传sql失败'); } });
	 */
}
uploadSql = function() {
	 ...
	 * Ext.MessageBox.alert('提示','请选择一条记录！'); } else{
	 */
					...
					bbar : [
							{
								...
								handler : function() {
									if (upload_view != null
											&& upload_view.isVisible()) {
										upload_view.hide();
										upload_view.setActive(true);
										uploadsql_flag = 1;
										var filePath = Ext.getCmp("sql_file")
												.getValue();
...
var query_databaseId = new Ext.form.ComboBox({
	...
	mode : 'remote', // 采用本地数据加载，因combox在创建时，已加载了数据。这样可自动匹配数据
	triggerAction : 'all',
	emptyText : '-请选择连接的数据库-',
	width : 120,
	// value:'mysql',
...
// 重置模糊查询
dataReset = function() {
	Ext.getCmp('query_databaseId').setValue('');
...
testSql = function() {
	...
	var sqlParam = afp.form.findField('sqlParam_add').getValue();
	...
	Ext.Ajax.request({
		method : 'post',
		url : CTX + '/sqlmgr/testSql.itm',
		params : {
			sqlSentence : Ext.util.Format.htmlEncode(sqlSentence),
...
testSql1 = function() {
	...
	var sqlParam = efp.form.findField('sqlParam_edit').getValue();
	if (sqlSentence.trim().length < 1) {
		Ext.Msg.alert('提示', '请填写sql语句');
		return;
	}
...
saveSubmit = function() {
	...
	if (databaseId == 29 && sqlParam.trim().length < 1) {
		Ext.Msg.alert('提示', 'hive的sql必须填写表头sqlparam');
		return;
	}
	...
	Ext.Ajax.request({
		method : 'post',
		url : CTX + '/sqlmgr/addOrEditSql.itm',
		params : {
			subject : subject,
			// ruleSql: ruleSql,
			ruleSql : Ext.util.Format.htmlEncode(ruleSql),
			sqlParam : sqlParam,
			system : system,
			proName : proName,
			excelName : excelName,
		...
		success : function(result) {
			...
			if (r == '1') {// 添加成功
				...
				// 进入上传sql文件的方法
				/*
				 * if(uploadsql_flag==1&&id!='-1'){ saveSqlSubmit(id); }
				 * uploadsql_flag==0;
				 */
				addwin.hide();
...
editSaveSubmit = function() {
	...
	var sqlParam = efp.form.findField('sqlParam_edit').getValue(); // ruleSql
...
// 删除记录方法
/*
 * delfun = function (){ //判断是否选择了一条记录
 * if(grid.getSelectionModel().getCount()!=1){
 * Ext.MessageBox.alert('提示','请选择一条记录！'); } else{ //获取grid record var record =
 * grid.getSelectionModel().getSelected(); var id = record.get('id');
 * Ext.Msg.confirm('提示','确定要删除吗？',function(btn){ if(btn=='yes'){
 * Ext.Ajax.request({ url: 'deviceconfig.itm?method=delDeviceInfo', method:
 * 'post', params:{ id: id }, success : function(result) { responseStr =
 * result.responseText; if(responseStr == "1"){ Ext.Msg.alert('提示','删除成功'); }
 * else if(responseStr == "-1"){ Ext.Msg.alert('提示','该设备有相关联的自动维护作业计划任务<br>请删除相关任务后再删除该设备'); }
 * else if(responseStr == "-2"){ Ext.Msg.alert('提示','该设备有相关联的性能指标采集任务<br>请删除相关任务后再删除该设备'); }
 * else{ Ext.Msg.alert('提示','操作未成功'); } } }); } });
 *  } }
 */
...
addfun = function() {
	...
	if (afp == null) {
		...
		afp = new Ext.form.FormPanel({
			...
			items : [ // 使用fieldset将表单值包起来
			{
				...
				items : [ {
					...
					items : [ new Ext.form.ComboBox({
						...
						valueField : 'id', // combox id值
						displayField : 'discribe', // combox 显示值
						typeAhead : true, // 自动匹配。注：typeAheadDelay
											// 参数--当typeAhead=true时
			...
			// 测试sql
				 ...
				 * '-',{ text:'上传sql', iconCls:'view', handler: uploadSql
				 * //上传sql },
				 */'->', {
				text : '保存',
				iconCls : 'save',
	...
	var top = document.body.scrollTop
			+ (document.body.clientHeight / 2 - addwin.height / 2);
...
editfun = function() {
	...
	if (grid.getSelectionModel().getCount() != 1) {
		...
		if (efp == null) {
			efp = new Ext.form.FormPanel({
				...
				// 测试sql
					 ...
					 * '-',{ text:'上传sql', iconCls:'view', handler: uploadSql
					 * //上传sql },'-',{ text:'下载sql', iconCls:'view', handler:
					 * function(){//下载sql if(downloadsql_flag==1){
					 * window.location.href = 'upLoad.itm?id='+record.get('id');
					 * }else{ Ext.Msg.alert('提示','该条记录没有sql'); }
					 *  } },
					 */'->', {
					text : '保存',
					iconCls : 'save',
		...
		var top = document.body.scrollTop
				+ (document.body.clientHeight / 2 - editwin.height / 2);
		editwin.setPosition(
				(document.body.clientWidth / 2 - editwin.width / 2), top);
		editwin.show();
		...
		efp.form.findField('sqlParam_edit').setValue(record.get('sqlParam'));
		...
		Ext.getCmp('databaseId1').setValue(record.get('dbName'));
		...
		Ext.Ajax.request({
			...
			success : function(result) {
				...
				Ext.getCmp('ruleSql_edit').setValue(resultStr.ruleSql);
	...
	if (grid.getSelectionModel().getCount() != 1) {
		...
		Ext.Ajax.request({
			...
			success : function(result) {
				...
				Ext.getCmp('sqlParam_view').setValue(resultStr.sqlParam);
				// 设置结果值到textArea中
				Ext.getCmp('ruleSql_view').setValue(resultStr.ruleSql);
...
function startJob(id, jobStatus) {
	...
	Ext.Ajax.request({
		method : 'post',
		url : CTX + '/jobmgr/scheduleJob.itm',
		params : {
			jobId : id,
			type : 'sql',
...
function showbutton(value, cellmeta, record) {
	...
	var jobStatus = record.data.jobState;
	// alert(value); +"<INPUT type=button value=执行sql
	// onclick=exceSql('"+value+"')>"
	var returnStr = "<INPUT type=button value=开始job onclick=startJob('" + value
			+ "'," + "'" + jobStatus + "')>"
			+ "<INPUT type=button value=停止job onclick=stopJob('" + value + "',"
			+ "'" + jobStatus + "')>";
	return returnStr;
}
Ext.onReady(function() {
	...
	Ext.BLANK_IMAGE_URL = CTX
			+ '/resources/js/ext/resources/images/default/s.gif';

	// 查询列表stroe 加载
	store = new Ext.data.SimpleStore({
		url : CTX + '/sqlmgr/querySqlInfo.itm',
		totalProperty : 'totalCount',
		root : 'rows',
	 ...
	 * reader: new Ext.data.JsonReader({totalProperty:'totalProperty',root:''},[
	 * {name: 'id'}, {name: 'index'}, {name: 'subject'}, {name: 'ruleSql'},
	 * {name: 'system'}, {name: 'createTime'}, {name: 'proName'}, {name:
	 * 'excelName'}, {name: 'sheetName'}, {name: 'jobState'} ])
	 */
	});
	if (theUser != 'bi_dev') {
		...
		grid = new Ext.grid.GridPanel({
			...
			// 查看函数
				 ...
				 * ,'-',{ text:'删除', tooltip:'删除', iconCls:'remove',
				 * handler:delfun //删除函数 }
				 */, '-', {
				text : '刷新',
				tooltip : '刷新',
				 ...
				 * ,'-',{ text:'上传sql', tooltip:'上传sql', iconCls:'add',
				 * handler:uploadSql }
				 */],
	...
	var panel_ = new Ext.Panel({
		...
		iconCls : 'query',
		layout : 'fit',
		tbar : [ '数据库：', query_databaseId, 'ID：', query_id, 'sql描述：',
				query_subject, 'sql语句片段：', query_sql, 'sql创建时间起：', query_date1,
				'sql创建时间止：', query_date2, {
					text : '查询',
					tooltip : '提交查询',
					iconCls : 'view',
					handler : function() {
