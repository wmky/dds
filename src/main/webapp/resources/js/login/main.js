...
//初始化菜单，静态，写死
					...
					var node1 = new Ext.tree.TreeNode({id:'301',text:'sql语句配置',leaf:true,href:CTX+'/sqlmgr/init.itm?url=sql/sql_mgr'});
					var node2 = new Ext.tree.TreeNode({id:'302',text:'发送邮件配置',leaf:true,href:CTX+'/sqlmgr/goMail.itm?url=sql/mail_mgr'});
//					var node3 = new Ext.tree.TreeNode({id:'303',text:'数据库配置',leaf:true,href:CTX+'/dbmgr/init.itm?url=dbmgr/db_mgr'});
	                ...
	                items:
	               		new Ext.TabPanel({
	                        ...
	                        items:[{
	                        	id:'tabPanel_301',
	                        	title:'sql语句配置',
	                        	html:'<iframe id="frame_fp" frameborder="0" style="width:100%; height:100%" src="'+CTX+'/sqlmgr/init.itm?url=sql/sql_mgr"></iframe>'
	                        }]
  				...
  				if(btn=='yes'){
	  				Ext.Ajax.request({
	  					...
	  					success : function(result) {
	  						...
	  						if(para[0]=='1'){
	  							alert(para[1]);
	  							window.location.href=CTX + '/loginctrl/init.itm';	//跳转到登录页面
	  						}
	  						else{
	  							alert(para[1]);
	  						}
